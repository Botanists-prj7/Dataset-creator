...
def print_map_of_occurences(column:str, crs: str, geodataframe_csv_location:str = None, geodataframe:gpd.GeoDataFrame = None):
    if geodataframe is not None:
        geodataframe_results = geodataframe
    elif geodataframe_csv_location is not None:
        geodataframe_results = csvTools.convert_csv_to_gdf(geodataframe_csv_location,True, crs)
    ...
    ax = geodataframe_results.plot(alpha=0.5,column = column, legend=True, cmap='PuBu', scheme='User_Defined',classification_kwds=dict(bins=[0,1]))
    cx.add_basemap(ax,source=cx.providers.CartoDB.Positron)
    plt.show()